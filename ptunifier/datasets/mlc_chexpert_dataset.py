...
class MLCCHEXPERTDataset(BaseDataset):
    def __init__(self, *args, split="", data_frac=1., **kwargs):
        assert split in ["train", "val", "test"]
        self.split = split

        if split == "train" and data_frac == 1.:
            names = ["mlc_chexpert_train"]
        elif split == "train" and data_frac == .1:
            names = ["mlc_chexpert_train_01"]
        elif split == "train" and data_frac == .01:
            names = ["mlc_chexpert_train_001"]
        elif split == "val":
            names = ["mlc_chexpert_val"]
        elif split == "test":
            names = ["mlc_chexpert_test"]
        else:
            raise ValueError
    ...
    def get_suite(self, index):
        ret = super(MLCCHEXPERTDataset, self).get_suite(index)
        img_index, cap_index = self.index_mapper[index]
        ret["mlc_labels"] = self.labels[img_index]
        return ret

    def collate(self, batch, mlm_collator):
        dict_batch = super(MLCCHEXPERTDataset, self).collate(batch, mlm_collator)
