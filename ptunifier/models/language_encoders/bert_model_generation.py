...
from ptunifier.models.prediction_heads import MLMHead
...
class BertGenerationDecoder(BertGenerationPreTrainedModel):
    def __init__(self, config, max_length, is_decoder=True, add_cross_attention=True):
        bg_config = BertGenerationConfig(
        ...
        super().__init__(bg_config)

        if not bg_config.is_decoder:
            logger.warning("If you want to use `BertGenerationDecoder` as a standalone, add `is_decoder=True.`")

        self.bert = BertGenerationEncoder(bg_config)
        self.lm_head = MLMHead(bg_config)
