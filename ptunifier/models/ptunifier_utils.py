...
from torchmetrics import F1, AUROC, BLEUScore
...
from ..metrics.my_metrics import Accuracy, Scalar, VQARADScore, COCOCaptionScore, JBScore
...
def set_metrics(pl_module):
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
            ...
            elif k == "mlm" or k == "umlm":
                setattr(pl_module, f"{split}_{k}_accuracy", Accuracy())
                setattr(pl_module, f"{split}_{k}_loss", Scalar())

            elif k == "mim" or k == "umim":
                setattr(pl_module, f"{split}_{k}_accuracy", Accuracy())
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
                ...
                if split == "train":
                    setattr(pl_module, f"{split}_{k}_aucroc", AUROC(
                        pl_module.hparams.config["label_size"][pl_module.hparams.config["label_column_name"]]))
                    ...
                    setattr(pl_module, f"{split}_{k}_f1", F1(
                        pl_module.hparams.config["label_size"][pl_module.hparams.config["label_column_name"]]))
                    setattr(pl_module, f"{split}_{k}_loss", Scalar())
                    setattr(pl_module, f"test_{k}_aucroc", AUROC(
                        pl_module.hparams.config["label_size"][pl_module.hparams.config["label_column_name"]]))
                    setattr(pl_module, f"test_{k}_f1", F1(
                        pl_module.hparams.config["label_size"][pl_module.hparams.config["label_column_name"]]))
                    setattr(pl_module, f"test_{k}_loss", Scalar())
                ...
                if split == "train":
                    ...
                    for i in [1, 2, 3, 4]:
                        setattr(pl_module, f"{split}_{k}_bleu_{i}", BLEUScore(n_gram=i))
                    setattr(pl_module, f"{split}_{k}_rouge", ROUGEScore())
                    setattr(pl_module, f"{split}_{k}_coco_caption", COCOCaptionScore())
                    setattr(pl_module, f"{split}_{k}_jb", JBScore())
                    setattr(pl_module, f"{split}_{k}_loss", Scalar())
                    for i in [1, 2, 3, 4]:
                        setattr(pl_module, f"test_{k}_bleu_{i}", BLEUScore(n_gram=i))
                    pl_module.test_clm_rouge = pl_module.val_clm_rouge
                    setattr(pl_module, f"test_{k}_coco_caption", COCOCaptionScore())
                    setattr(pl_module, f"test_{k}_jb", JBScore())
                    setattr(pl_module, f"test_{k}_loss", Scalar())
...
def epoch_wrapup(pl_module, test=False):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            value = getattr(pl_module, f"{phase}_{loss_name}_aucroc").compute()
            pl_module.log(f"{loss_name}/{phase}/aucroc_epoch", value)
            getattr(pl_module, f"{phase}_{loss_name}_aucroc").reset()

            pl_module.log(f"{loss_name}/{phase}/f1_epoch", getattr(pl_module, f"{phase}_{loss_name}_f1").compute())
            ...
            if not phase == "train":
                values = {}
                for i in [1, 2, 3, 4]:
                    kk = f"torchmetrics_bleu_{i}"
                    values[kk] = getattr(pl_module, f"{phase}_{loss_name}_bleu_{i}").compute()
                    pl_module.log(f"{loss_name}/{phase}/bleu_{i}_epoch", values[kk])
                    getattr(pl_module, f"{phase}_{loss_name}_bleu_{i}").reset()

                for kk, vv in getattr(pl_module, f"{phase}_{loss_name}_rouge").compute().items():
                    kk = "torchmetrics_" + kk
                    values[kk] = vv
                    pl_module.log(f"{loss_name}/{phase}/{kk}", vv)
                getattr(pl_module, f"{phase}_{loss_name}_rouge").reset()

                for kk, vv in getattr(pl_module, f"{phase}_{loss_name}_coco_caption").compute().items():
                    kk = "coco_caption_" + kk
                    values[kk] = vv
                    pl_module.log(f"{loss_name}/{phase}/{kk}", vv)
                getattr(pl_module, f"{phase}_{loss_name}_coco_caption").reset()

                for kk, vv in getattr(pl_module, f"{phase}_{loss_name}_jb").compute().items():
                    kk = "jb_" + kk
                    values[kk] = vv
                    pl_module.log(f"{loss_name}/{phase}/{kk}", vv)
                getattr(pl_module, f"{phase}_{loss_name}_jb").reset()

                if pl_module.hparams.config["vision_only"]:
                    value = values["coco_caption_BLEU_1"] + \
                            values["coco_caption_BLEU_2"] + \
                            values["coco_caption_BLEU_3"] + \
                            values["coco_caption_BLEU_4"]
                else:
                    value = values["torchmetrics_rouge1_fmeasure"] + \
                            values["torchmetrics_rouge2_fmeasure"] + \
                            values["torchmetrics_rougeL_fmeasure"]
...
def set_schedule(pl_module):
    ...
    head_names = ["mlm_head", "mim_head", "itm_head", "itc_head",
                  "vqa_head", "cls_head", "irtr_head", "mlc_head", "clm_head"]
    multi_modal_names = ['multi_modal', "pseudo"]
