...
class CheXbert(nn.Module):
    def __init__(self, refs_filename=None, hyps_filename=None, device="cuda", **kwargs):
        ...
        self.model = bert_labeler(pretrain_path="bert-base-uncased", inference=True)

        # Downloading pretrain model from huggingface
        checkpoint = "downloaded/scorers/chexbert.pth"
    ...
    def get_label(self, report, mode="rrg"):
        ...
        if self.device == "cpu":
            ...
            out = self.model(batch.cuda(), attn_mask.cuda())
