...
class MauveScorer(nn.Module):
    def __init__(self, model=None, device="cuda", **kwargs):
        super().__init__()
        if model is None:
            model = "distilgpt2"
        self.model = model
        ...
        self.model = get_model(self.model, self.tokenizer, (-1 if device == "cpu" else 0))
    ...
    def forward(self, refs, hyps):
        ...
        tokenized_hyps = [self.tokenizer.encode(sen, return_tensors='pt', truncation=True, max_length=512)
                          for sen in hyps]
        p_features = featurize_tokens_from_model(self.model, tokenized_refs, name="p",
                                                 batch_size=64).detach().cpu().numpy()
        q_features = featurize_tokens_from_model(self.model, tokenized_hyps, name="q",
                                                 batch_size=64).detach().cpu().numpy()
