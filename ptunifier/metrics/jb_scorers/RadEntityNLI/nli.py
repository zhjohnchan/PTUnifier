...
class _NLIScorer:
    ...
    def __init__(self, neutral_score=(1.0 / 3), batch=16, nthreads=2, pin_memory=False, bert_score=None,
                 num_layers=None, cache=None, verbose=False):
        self.model = None
        ...
        if bert_score is not None:
            self.bert_score_model = BERTScorer(model_type=bert_score, batch_size=batch, nthreads=nthreads,
                                               lang='en', rescale_with_baseline=True, num_layers=num_layers)
        else:
...
class SimpleNLI(_NLIScorer):
    def __init__(self, model, neutral_score=(1.0 / 3), batch=16, nthreads=2, pin_memory=False, bert_score=None,
                 num_layers=None, device="cuda", cache=None, verbose=False):
        super(SimpleNLI, self).__init__(neutral_score, batch, nthreads, pin_memory, bert_score, num_layers, cache,
                                        verbose)
        self.device = device
    ...
    @classmethod
    def load_model(cls, states=None, device="cuda"):
        bert_type = 'bert'
        name = 'bert-base-uncased'
        bertnli = BERTNLI(name, bert_type=bert_type, length=384, force_lowercase=True, device=device)
