...
def data_cuda(*tensors, device='cuda', non_blocking=False):
    if device == 'cuda':
        cuda_tensors = []
...
class BERTNLI(torch.nn.Module):
    ...
    def __init__(self, name_or_path, bert_type='bert', cls='linear', length=128, force_lowercase=False, device='cuda',
                 verbose=False):
    ...
    def forward(self, sent1s, sent2s):
        ...
        out = self.bert(token_ids, attention_mask=attn_mask, token_type_ids=seg_ids)
